...
- 心跳调度从固定周期改成名义 `300s`、带 `±90s` 轻微抖动
- 同一轮多个心跳目标之间加入随机错峰，避免稳定的连续发包节奏
- 页面状态说明会直接展示“名义 5 分钟（轻微抖动）”
- `.env.example`、README 和交接文档与线上心跳策略保持一致
...
- 心跳采样
  ...
  - 调度抖动 `±90s`
  - 同轮目标错峰 `0~8s`
  - 默认目标 `223.5.5.5 / 119.29.29.29 / 1.1.1.1`
- 心跳展示
  - 面板只保留 `5m / 1h` 两档
  - `7 天` 视图在 `1h` 模式下继续使用 `15m` 细格展示全天分布
- 正式测速
  - 固定执行时刻 `05:00 / 16:00`
  - 用于低打扰观察长期带宽和延迟变化
...
| `HEARTBEAT_JITTER_SECONDS` | `90` | 每轮心跳调度的随机抖动范围，单位秒，实际间隔会在基准值上下轻微波动 |
| `HEARTBEAT_TARGET` | `223.5.5.5` | 主心跳目标 |
| `HEARTBEAT_TARGET_SPREAD_SECONDS` | `8` | 同一轮多个心跳目标之间的最大错峰秒数 |
